...
// ===               MAIN LOOP              ===
// ================================================================
#define RECONNECTION_THRESHOLD 1000 //1s
long disconnected_Time, reconnected_Time, reconnection_Duration = 0; //tracks duration beetle takes to reconnect to relay node
...
void loop() {
  ...
  if (numShots > 0)
  {
    ...
    while (!is_ack)
    {
      ...
      {
        reconnected_Time = millis();
        reconnection_Duration = reconnected_Time - disconnected_Time; //calc duration beetle reconnects to relay node
        ...
        // keep numShots if time beetle takes to reconnect to relay node is less than 1s, else reset
        if (reconnection_Duration > RECONNECTION_THRESHOLD) {
           numShots = 0;
        }
        ...
        disconnected_Time = millis(); //beetle starts to disconnect from relay node
      }
    }
  }
